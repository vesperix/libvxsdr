...
namespace option_utils {
    ...
    struct option_as_string {
        ...
        public:
            ...
            template <typename T> std::string incompatible_type_error_message(const std::string& name, const supported_types type) const {
                return "incompatible types for option \"" + name + "\": cannot cast " + type_to_string(type) + " to (possibly mangled) type " + typeid(T).name();
            }
    ...
    struct parsed_options {
        ...
        public:
            parsed_options(const std::map<std::string, std::string>& vals, const std::map<std::string, supported_types>& tps, const bool throw_except) {
                values = vals;
                types = tps;
                throw_on_error = throw_except;
            };
    ...
    class program_options {
        ...
        public:
            ...
            void add_option(const std::string& long_name, const std::string& help_text, supported_types type, const std::string& default_value) {
                types[long_name] = type;
