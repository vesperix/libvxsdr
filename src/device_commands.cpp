...
bool vxsdr::imp::set_max_payload_bytes(const unsigned max_payload_bytes) {
    ...
    if (vxsdr::imp::send_packet_and_check_response(p, "set_max_payload_bytes()")) {
        ...
        return data_tport->set_max_samples_per_packet(max_samples_per_packet<std::complex<int16_t>>(max_payload_bytes));
    }
    return false;
}
...
std::optional<double> vxsdr::imp::get_sensor_reading(const std::string& sensor_name, const uint8_t subdev) {
    ...
    strncpy((char*)p.name, sensor_name.c_str(), MAX_CMD_RSP_PAYLOAD_BYTES - 1);
