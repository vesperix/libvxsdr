...
udp_data_transport::udp_data_transport(const std::map<std::string, int64_t>& settings,
    ...
    if (config.count("udp_data_transport:mtu_bytes") > 0) {
        ...
        if (specified_max_samples < max_samples_per_packet) {
            if (set_max_samples_per_packet(specified_max_samples)) {
                LOG_INFO("reducing max_samples_per_packet to {:d} on udp data sender socket (specified mtu)",
    ...
    if (mtu_est < 0) {
        ...
        if (socket_max_samples < max_samples_per_packet) {
            if (set_max_samples_per_packet(socket_max_samples)) {
                LOG_INFO("reducing max_samples_per_packet to {:d} on udp data sender socket (socket mtu)",
    ...
    tx_data_queue = std::make_unique<vxsdr_queue<data_queue_element>>(config["udp_data_transport:tx_data_queue_packets"]);
...
size_t udp_data_transport::packet_send(const packet& packet, int& error_code) {
    ...
    while (true) {
        ...
        if (bytes == 0 and err == net_error_code_types::no_buffer_space) {
