...
size_t vxsdr::put_tx_data(const std::vector<std::complex<int16_t>> &data, const uint8_t subdev, const double timeout_s) {
    return p_imp->put_tx_data<int16_t>(data, subdev, timeout_s);
}

size_t vxsdr::put_tx_data(const std::vector<std::complex<float>> &data, const uint8_t subdev, const double timeout_s) {
    return p_imp->put_tx_data<float>(data, subdev, timeout_s);
}
